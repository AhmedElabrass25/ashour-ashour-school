...
export function LeadershipPage({ submissions }: LeadershipPageProps) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-slate-50 text-slate-900 font-sans" dir="rtl">
      {/* Header */}
      <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur-md">
        <div className="w-[min(1160px,calc(100%-32px))] mx-auto min-h-[64px] flex items-center justify-between gap-4">
          <a className="flex items-center gap-2.5 shrink-0 text-slate-900 no-underline" href="#الرئيسية" onClick={closeMenu}>
            <span className="w-9 h-9 grid place-items-center rounded-sm bg-blue-600 text-white font-bold shadow-2xs">
              <Building2 size={18} />
            </span>
            <span>
              <strong className="block text-sm font-bold text-slate-800">الإدارة التعليمية</strong>
              <small className="block text-[10px] text-slate-500 font-bold uppercase tracking-wider">القيادة التعليمية</small>
            </span>
          </a>

          {/* Desktop Nav */}
          <nav className="hidden md:flex justify-center items-center gap-6 flex-1">
            {["الرئيسية", "النبذة", "المسؤوليات", "الخبرات", "الصور", "التواصل"].map((item) => (
              <a key={item} href={`#${item}`} onClick={closeMenu} className="text-slate-600 text-xs font-bold hover:text-blue-600 transition-colors">
                {item}
              </a>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center">
            <button className="inline-flex items-center justify-center gap-2 h-9 border-0 rounded-sm px-4 font-bold text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-2xs transition-colors" type="button" onClick={goToDashboard}>
              لوحة البيانات <ChevronLeft size={14} />
            </button>
          </div>

          {/* Mobile Menu Button */}
          <button className="md:hidden grid place-items-center border border-slate-200 rounded-sm bg-white text-blue-600 p-2 shadow-2xs" type="button" aria-label={menuOpen ? "إغلاق القائمة" : "فتح القائمة"} aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}>
        ...
        <div className={`absolute top-full right-0 left-0 bg-white border-b border-slate-200 shadow-md overflow-hidden transition-all duration-200 ${menuOpen ? "opacity-100 scale-y-100" : "opacity-0 scale-y-0 h-0"}`}>
          <div className="flex flex-col p-4 gap-1">
            {["الرئيسية", "النبذة", "المسؤوليات", "الخبرات", "الصور", "التواصل"].map((item) => (
              <a key={item} href={`#${item}`} onClick={closeMenu} className="py-2 px-3 text-slate-700 text-xs font-bold hover:bg-blue-50 hover:text-blue-700 rounded-sm transition-colors">
                {item}
              </a>
            ))}
      ...
      {/* Hero Section */}
      <section className="py-16 md:py-24 bg-white" id="الرئيسية">
        <div className="w-[min(1160px,calc(100%-32px))] mx-auto grid md:grid-cols-[1fr_0.85fr] gap-10 items-center">
          <div className="order-2 md:order-1 flex flex-col items-start">
            <span className="inline-flex items-center gap-2 text-blue-700 text-xs font-bold uppercase tracking-wider mb-3 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-sm">
              <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
              بوابة القيادة
            </span>
            <h1 className="my-2 text-3xl md:text-4xl lg:text-5xl font-extrabold text-slate-900 leading-tight">
              {profile.name}
            </h1>
            <h2 className="m-0 text-lg md:text-xl font-bold text-blue-600">
              {profile.position}
            </h2>
            <p className="mt-1 text-amber-600 text-xs font-bold tracking-wide">
              {profile.department}
            </p>
            <p className="max-w-[540px] mt-4 text-slate-600 text-sm leading-relaxed">
              {profile.bio}
            </p>
            <div className="flex flex-col sm:flex-row gap-3 mt-8 w-full sm:w-auto">
              <a className="inline-flex items-center justify-center gap-2 h-10 rounded-sm px-6 font-bold text-xs bg-blue-600 text-white hover:bg-blue-700 shadow-2xs transition-colors w-full sm:w-auto" href="#النبذة">
                التعرف على المزيد <ChevronLeft size={16} />
              </a>
              <a className="inline-flex items-center justify-center gap-2 h-10 rounded-sm px-6 font-bold text-xs border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition-colors w-full sm:w-auto" href="#التواصل">
                التواصل المباشر
              </a>
            </div>
            <div className="flex items-center gap-2 mt-6 text-slate-500 text-xs font-semibold">
              <ShieldCheck size={16} className="text-emerald-500" />
              <span>صفحة رسمية معتمدة عبر نظام الإدارة التعليمية</span>
            </div>
          </div>
          
          <div className="order-1 md:order-2 relative min-h-[300px] md:min-h-[400px]">
            <div className="overflow-hidden rounded-sm border border-slate-200 bg-slate-100 shadow-sm h-full">
              <ProfileImage src="/leader-image.jpg" alt="صورة المدير" />
            </div>
            <div className="absolute right-2 bottom-2 z-10 flex items-center gap-3 p-3 rounded-sm bg-white border border-slate-200 shadow-md">
              <div className="w-8 h-8 rounded-sm bg-blue-50 grid place-items-center text-blue-600 font-bold">
                <GraduationCap size={18} />
              </div>
              <div>
                <strong className="block text-xs font-bold text-slate-800">{profile.position}</strong>
                <small className="block text-[10px] text-slate-500 font-semibold">{profile.department}</small>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-slate-50 border-t border-slate-200" id="النبذة">
        <div className="w-[min(1160px,calc(100%-32px))] mx-auto grid md:grid-cols-[0.8fr_1fr] gap-10 items-center">
          <div className="h-[300px] md:h-[400px] overflow-hidden rounded-sm border border-slate-200 bg-white shadow-2xs">
            <img src={gallery[1].src} alt="صورة ثانية" className="w-full h-full object-cover" />
          </div>
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-slate-800 mb-4 leading-tight">مسيرة مهنية رائدة في خدمة وتطوير التعليم</h2>
            ...
            <p className="text-slate-600 text-sm leading-relaxed border-r-4 border-amber-400 pr-4 bg-amber-50/60 py-2.5 rounded-sm">
            ...
            <div className="flex flex-wrap gap-4 mt-6 p-3 bg-white rounded-sm border border-slate-200">
              <span className="inline-flex items-center gap-2 text-slate-800 text-xs font-bold bg-slate-50 px-2.5 py-1 rounded-sm border border-slate-200">
